#!/bin/bash
# AutoPaqet Server for Ubuntu
# Usage: curl -sSL <url> | sudo bash
#    or: sudo bash autopaqet-server.sh
...
info "Starting AutoPaqet server setup..."
...
info "Setting up required packages..."
...
# Step 4: Clone and build AutoPaqet
info "Cloning AutoPaqet repository..."
rm -rf "$INSTALL_DIR"
git clone --depth 1 "$AUTOPAQET_REPO" "$INSTALL_DIR" 2>/dev/null
cd "$INSTALL_DIR"

info "Building AutoPaqet..."
...
chmod +x "$BINARY_PATH"
success "AutoPaqet built and ready at $BINARY_PATH"
...
cat > /etc/systemd/system/autopaqet.service << 'EOF'
[Unit]
...
systemctl enable autopaqet >/dev/null 2>&1
systemctl start autopaqet
success "Systemd service created and started"

# Step 9: Verify service is running
sleep 2
if systemctl is-active --quiet autopaqet; then
    success "AutoPaqet server is running!"
else
    error "AutoPaqet service failed to start. Check: journalctl -u autopaqet"
fi

# Step 10: Output client configuration
echo ""
echo -e "${GREEN}═══════════════════════════════════════════════════════════════${NC}"
echo -e "${GREEN}              AUTOPAQET SERVER SETUP COMPLETE                  ${NC}"
echo -e "${GREEN}═══════════════════════════════════════════════════════════════${NC}"
echo ""
...
echo "  Check status:      systemctl status autopaqet"
echo "  View logs:         journalctl -u autopaqet -f"
echo "  Restart service:   systemctl restart autopaqet"
echo "  Stop service:      systemctl stop autopaqet"
echo ""
echo -e "${GREEN}═══════════════════════════════════════════════════════════════${NC}"
